...
void ST7789_SetWindow(uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
    ...
    // Send RAMWR command to set column and page registers to the specified positions
    ST7789_SendCommand(ST7789_RAMWR);
}

void ST7789_WritePixelData(uint8_t *data, uint32_t length) {
    ST7789_SendData(data, length);
}
...
//     // Prepare a fixed buffer of 64 pixels (128 bytes) - write data in chunks of 64 pixels to optimize SPI transfers
//     uint8_t data[128]; // 64 pixels * 2 bytes/pixel
//     for (uint32_t i = 0; i < 64; i++) {
//         // Byte 1: RRRRRGGG
//         // Byte 2: GGGBBBBB
...
// void ST7789_DrawChar(uint32_t x, uint32_t y, const Font_TypeDef* font, char ch, uint16_t color, uint16_t bg_color) {
